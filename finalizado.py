...
def oi():
    ...
    total["A20"] = "TOTAL"
    total["A20"].fill = PatternFill(start_color="90ee90", end_color="90ee90", fill_type="solid")
    total["A20"].font = Font(bold=16)
    ...
    merendas = arquivo.active
    merendas["A18"] = "Lanches"
    merendas["A18"].fill = PatternFill(start_color="808080", end_color="808080", fill_type="solid")
    merendas["A18"].font = Font(bold=16)
    
    gasto = arquivo.active
    gasto["A19"] = "Gasto extra"
    gasto["A19"].font = Font(bold=16)
    ...
    for col1 in range(2, 14):  
        celula_total1 = aba.cell(row=20, column=col1)
        celula_total1.value = f"=SUM({chr(64 + col1)}12:{chr(64 + col1)}19)"
    ...
    for linha in range(6, 9):  # Linhas 1 a 5
        for coluna in range(2, 14):  # Colunas 1 a 3
            celula = aba.cell(row=linha, column=coluna)  # Exemplos de valores
            celula.number_format = '"R$"#,##0.00' 
            celula.alignment = Alignment(horizontal="center", vertical="center")
    
    for linha1 in range(12, 20):  # Linhas 1 a 5
        for coluna3 in range(2, 14):  # Colunas 1 a 3
            celula4 = aba.cell(row=linha1, column=coluna3)  # Exemplos de valores
            celula4.number_format = '"R$"#,##0.00' 
            celula4.alignment = Alignment(horizontal="center", vertical="center")
    
    
    for col2 in range(1, 18 + 1): 
        celula5 = aba.cell(row=1, column=col2)
        celula5.alignment = Alignment(horizontal="center", vertical="center")
